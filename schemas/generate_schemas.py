...
def to_schema(fname, *ds):
    ...
    with open(path.join(schemas_dir,
                        fname.replace('.yml', '.schema.json')), 'wt') as f:
...
def seq1_endswith0(sequence, endswith):
    return (two_d[1] for two_d in sequence
            if path.basename(two_d[0]).endswith(endswith))

if __name__ == '__main__':
    tuple(remove(path.join(schemas_dir, f)) for f in listdir(schemas_dir)
          if f.endswith('.json'))
    ymls = tuple(as_dict(path.join(schemas_dir, f))
